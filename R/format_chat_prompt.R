#' Format a ChatGPT prompt (particularly useful for classification tasks)
#'
...
#'
#' @return Returns a series of messages formatted as a list object, which can be used as an input for openai::create_chat_completion().
#' @export
#'
#' @examples
#' data(scotus_tweets_examples)
#'
#' format_chat_prompt(text = "I am disappointed with this ruling.",
#'                    instructions = "Decide whether the sentiment of this statement is Positive or Negative.",
#'                    examples = scotus_tweets_examples)
format_chat_prompt <- function(text, instructions = NA,
                               examples = data.frame(),
                               system_message = NA){
